...
async def get_buying_list(message):
    with open ('hw53/muka.JPG', 'rb') as img_1:
        await message.answer_photo (img_1,'Название: Кусок муки, описание: сыпучий, цена может меняться, цена: 100р')
    with open ('hw53/koshka.JPG', 'rb') as img_2:
        await message.answer_photo (img_2, 'Название: Кошка (красивое), описание: иногда можно гладить, цена: не продаём, показываем')
    with open ('hw53/stameska.JPG', 'rb') as img_3:
        await message.answer_photo (img_3, 'Название: Стамеска (на развес), описание: обычная стамеска, цена: обратитесь к админу: @durov')
    with open ('hw53/kusok.JPG', 'rb') as img_4:
        await message.answer_photo (img_4, 'Название: Куски кода, описание: стабильно не рабочие, цена: 50, для пентагона 10**6')
