...
@Ds.message_handler(text=('Регистрация'))
async def sign_up(message):
    await message.answer(text='Ваш никнейм')
    await Registration_state.username.set()

@Ds.message_handler(state= Registration_state.username)
async def mail(message, state):
    username = message.text  
    if is_include(username):  
        await message.answer('Пользователь с таким ником уже существует')
    else:
        await state.update_data(username=username)  
        await message.answer('Введите свой email')
        await Registration_state.email.set()

@Ds.message_handler(state=Registration_state.email)
async def age(message, state):
    email = message.text  
    await state.update_data(email=email)  
    await message.answer('Введите возраст')
    await Registration_state.age.set()

@Ds.message_handler(state= Registration_state.age)
async def fin_reg(message, state):
    age = message.text
